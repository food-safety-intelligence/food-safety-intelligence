...
def _warm():
    ...
    files = [
        ...
        (
            os.environ.get("HISTORY_JSON_PATH", "/tmp/inspection_history.json"),
            f"{prefix}/inspection_history.json",
        ),
    ]
    ...
    _logger.warning(
        f"[warm] S3 download failed in all regions (last: {last_error}). "
...
# Imported after the sys.path setup and _load() calls above so the tool
# subdirectories (and model.load) resolve.
from bedrock_agentcore.runtime import BedrockAgentCoreApp  # noqa: E402
from model.load import load_model  # noqa: E402
...
async def invoke(payload, context):
    ...
    if not query:
        yield "Error: query field is required"
        return
