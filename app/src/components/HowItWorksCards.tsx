        ...
        <table className="w-full text-sm border-collapse table-fixed">
          <thead>
            <tr className="text-left text-sage text-xs tracking-[0.08em] uppercase border-b border-line bg-tint/40">
              ...
              <th className="py-2.5 px-4 font-medium text-right">Flagged</th>
              <th className="py-2.5 px-4 font-medium text-right">Precision (hit rate)</th>
              <th className="py-2.5 px-4 font-medium text-right">Recall (events caught)</th>
            </tr>
          </thead>
          <tbody className="num text-ink/85">
            {rows.map((p) => (
              <tr key={p.frac} className="border-b border-line last:border-b-0">
                ...
                <td className="py-2.5 px-4 text-right">{p.n_flagged.toLocaleString("en-US")}</td>
                <td className="py-2.5 px-4 text-right">{Math.round(p.precision * 100)}%</td>
                <td className="py-2.5 px-4 text-right">{Math.round(p.recall * 100)}%</td>
              </tr>
            ))}
          </tbody>
        </table>
