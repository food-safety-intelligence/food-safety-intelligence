...
// NYC-specific definitions; the cross-city terms come from SHARED_GLOSSARY.
const NYC_GLOSSARY: GlossaryEntry[] = [
  ...
  { id: "pr-auc", term: "PR-AUC / ROC-AUC", short: "Ranking-quality scores. PR-AUC rewards finding the minority (B/C) cases; ROC-AUC is base-rate independent, so it is comparable across cities. NYC's ROC-AUC ~0.66 is a modest, preview-level signal." },
...
export function HowItWorksNyc() {
  ...
  return (
    <div>
      ...
      >
        New York City is a research-preview second city. Each score is a calibrated
        probability that an establishment&apos;s{" "}
        <strong className="text-ink font-medium">next inspection is graded B or C</strong>{" "}
        under New York&apos;s letter-grade system, produced by a batch-scored-to-JSON
        pipeline (calibrated model plus SHAP drivers) on New York data.
      </MethodologyHero>
      ...
      <div className="mt-6 space-y-8">
        ...
        <article>
          ...
          <p className="text-sm text-muted leading-relaxed mt-1.5 max-w-[62ch]">
            ...
            map, list, and detail pages. They&apos;re set from NYC&apos;s own score
            distribution, so a high base rate doesn&apos;t push most places into the
            upper bands.
          </p>
          {m && (
      ...
      <div className="mt-10 space-y-8">
        ...
        <article>
          ...
          <p className="text-muted leading-[1.7] mt-3 max-w-[62ch]">
            ...
            inspection, is the grade B or C (score ≥ 14)? The label is anchored to
            the next inspection whenever it occurs, because NYC&apos;s ~annual cadence
            makes a short fixed window empty.
        ...
        <article>
          ...
          {m && (
            <p className="text-muted leading-[1.7] mt-3 max-w-[62ch]">
              ...
              These are modest numbers, which is why NYC is labelled a preview: the
              model ranks risk better than chance, but with real uncertainty.
            </p>
          )}
          {m && (
            <dl className="mt-4 grid gap-2.5 sm:grid-cols-2 max-w-[62ch] text-sm">
              <div className="rounded-xl border border-line bg-card px-3.5 py-2.5">
                ...
                <dd className="text-muted leading-snug mt-0.5">How often it ranks a B/C inspection above a clean one. Base-rate-independent, so it is the fair cross-city measure; NYC&apos;s is a modest, preview-level signal.</dd>
              </div>
              <div className="rounded-xl border border-line bg-card px-3.5 py-2.5">
                <dt className="font-medium text-ink">PR-AUC {m.headline.pr_auc.toFixed(2)}</dt>
                <dd className="text-muted leading-snug mt-0.5">Ranking quality for the B/C class. Its floor is the {prevPct}% base rate: NYC&apos;s is high, so this number looks strong but mostly reflects how common B/C is, not skill. Read it against that floor, not across cities.</dd>
              </div>
              <div className="rounded-xl border border-line bg-card px-3.5 py-2.5">
        ...
        limitations={
          <div className="space-y-4 text-muted leading-[1.7]">
            <p>
              ...
              upgrade.</strong> Its signal is modest (ROC-AUC ~0.66; lift ~1.6×), so
              treat NYC scores as a rougher guide.
            </p>
            <p>
              ...
              per-establishment history is thin and the trend chart is sparse.
            </p>
            <p>
