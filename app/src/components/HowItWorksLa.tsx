...
import { BookMarked, BookOpen, Target, Wrench } from "lucide-react";
import { useEffect, useState } from "react";
import { dataUrl } from "@/lib/city";
import type { RiskTier } from "@/lib/scores";
import { TierPill } from "@/components/TierPill";
import {
  ModelCard, ModelExplainer, DataGovernance, MethodologyHero, articleFor,
  SectionLabel, HeroStat, WaterfallRow, HIW_NAV, SHARED_GLOSSARY, type GlossaryEntry,
} from "@/components/HowItWorksCards";
...
// LA-specific definitions; the cross-city terms come from SHARED_GLOSSARY.
const LA_GLOSSARY: GlossaryEntry[] = [
  ...
  { id: "pr-auc", term: "PR-AUC / ROC-AUC", short: "Ranking-quality scores. PR-AUC rewards finding the minority (B/C) cases; ROC-AUC is base-rate independent, so it is comparable across cities. LA's ROC-AUC ~0.74 is a moderate, preview-level signal." },
...
export function HowItWorksLa() {
  ...
  return (
    <div>
      ...
      >
        Los Angeles County is a research-preview third city. Each score is a
        calibrated probability that an establishment&apos;s{" "}
        <strong className="text-ink font-medium">next inspection is graded B or C</strong>{" "}
        under LA&apos;s letter-grade system, produced by a batch-scored-to-JSON
        pipeline (calibrated model plus SHAP drivers) on LA data.
      </MethodologyHero>
      ...
      <div className="mt-6 space-y-8">
        ...
        <article>
          ...
          <p className="text-sm text-muted leading-relaxed mt-1.5 max-w-[62ch]">
            ...
            map, list, and detail pages. They&apos;re set from LA&apos;s own score
            distribution: LA&apos;s B/C base rate is low (~{prevPct}%), so the cutoffs
            are tuned to that.
          </p>
          {m && (
        ...
        <article>
          ...
          <p className="text-muted leading-[1.7] mt-3 max-w-[62ch]">
            ...
            so the trend is sparse and often reads
      ...
      <div className="mt-10 space-y-8">
        ...
        <article>
          ...
          <p className="text-muted leading-[1.7] mt-3 max-w-[62ch]">
            ...
            inspection, does the grade drop to B or C (score below 90)? The label is
            anchored to the next inspection whenever it occurs, because LA&apos;s
            ~annual cadence makes a short fixed window empty. Source:{" "}
      ...
      <div className="mt-10 space-y-8">
        ...
        <article>
          ...
          {m && (
            <p className="text-muted leading-[1.7] mt-3 max-w-[62ch]">
              ...
              These are moderate numbers: LA ranks risk better than chance, but it is
              still a preview rather than a production model.
            </p>
          )}
          {m && (
            <dl className="mt-4 grid gap-2.5 sm:grid-cols-2 max-w-[62ch] text-sm">
              <div className="rounded-xl border border-line bg-card px-3.5 py-2.5">
                ...
                <dd className="text-muted leading-snug mt-0.5">How often it ranks a B/C inspection above a clean one. Base-rate-independent, so it is the fair cross-city measure; LA&apos;s is a moderate, preview-level signal.</dd>
              </div>
              <div className="rounded-xl border border-line bg-card px-3.5 py-2.5">
                <dt className="font-medium text-ink">PR-AUC {m.headline.pr_auc.toFixed(2)}</dt>
                <dd className="text-muted leading-snug mt-0.5">Ranking quality for the rare B/C class. Its floor is the {prevPct}% base rate; read it against that floor rather than across cities with different base rates.</dd>
              </div>
              <div className="rounded-xl border border-line bg-card px-3.5 py-2.5">
        ...
        <article>
          ...
          <p className="text-md text-muted leading-relaxed mt-2 max-w-[62ch]">
            ...
            for one real high-risk establishment (risk score 56).
          </p>
          <div className="mt-4 rounded-2xl border border-line bg-card overflow-hidden max-w-[62ch]">
            <WaterfallRow label="Base rate (model intercept)" value={-2.04} muted />
            <WaterfallRow label="Current inspection score: 73/100" value={0.70} />
            <WaterfallRow label="Average past inspection score: 77.5/100" value={0.45} />
            <WaterfallRow label="2 prior inspections on record" value={-0.39} />
            <WaterfallRow label="3 food contact surface violations at this inspection" value={0.38} />
            <WaterfallRow label="18 violations at this inspection" value={0.26} />
            <WaterfallRow label="Everything else (remaining features)" value={0.90} muted />
            <WaterfallRow label="Total (calibrated log-odds)" value={0.26} strong />
            <div className="flex items-center justify-between gap-3 px-4 py-2.5 bg-tint/50">
              ...
              <span className="num text-terra-strong font-semibold">56.4%</span>
            </div>
          </div>
        ...
        limitations={
          <div className="space-y-4 text-muted leading-[1.7]">
            <p>
              ...
              upgrade.</strong> Its signal is moderate (ROC-AUC ~0.74; lift ~2.1×), so
              treat LA scores as a rougher guide.
            </p>
            <p>
              ...
              opposite of the usual lower-is-worse scale). The risk score already
              accounts for this; it is always
