...
misplace a map pin or produce a wrong 311 count. We **flag and null** such
coordinates (warn, then set them to NaN) but **keep the row**: a bad geocode is
a broken display field, not a reason to drop a valid inspection (real label, real
prior_* history, a real restaurant). Nulling routes the bad point into the same
"missing geo" path every consumer already handles — the map skips the pin
(`getMapPins` filters null/NaN) and the 311 join counts 0 — instead of plotting
it in Lake Michigan or computing a wrong spatial count. The warning still
surfaces the upstream geocoding bug so it gets fixed rather than silently hidden.
...
def flag_and_null_out_of_bbox(
    ...
    Warns with the count so the upstream geocoding bug surfaces. Nulling — not
    dropping — keeps the valid inspection and routes the bad point into the
    existing missing-geo path (map skips pin, 311 counts 0).
    """
    ...
    if n:
        warnings.warn(
            ...
            "those coordinates (row kept — map pin skipped, 311 counts 0). "
