"""A/B experiment: building features re-enablement test.

Trains two XGBoost models on the same temporal split:
  A (control):   ALL_FEATURES (current production set)
  B (treatment): ALL_FEATURES + BUILDING_FEATURES

Reports val/test metrics side-by-side (PR-AUC, precision@10%, ROC-AUC) and the
delta. Also runs expanding-window CV on train to check fold-level consistency.

Prerequisites:
  - features parquet built WITH --with-buildings:
      PYTHONPATH=src uv run python scripts/build_features.py --with-buildings \\
          --out data/processed/features/features_with_buildings.parquet
  - OR: the default features parquet already contains building columns (e.g.
    if built from a notebook that included building data).

Run:
    PYTHONPATH=src uv run python scripts/experiment_building_features.py
    PYTHONPATH=src uv run python scripts/experiment_building_features.py \\
        --features data/processed/features/features_with_buildings.parquet
"""
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help="path to features parquet (must include building columns for treatment arm)",
    )
    ...
    # Check building columns are present
    missing_bldg = [c for c in BUILDING_FEATURES if c not in features.columns]
    if missing_bldg:
        raise SystemExit(
            f"Missing {len(missing_bldg)} building columns: {missing_bldg}. "
            "Rebuild with: scripts/build_features.py --with-buildings"
        )

    # Filter right-truncated rows
    ...
    features_b = ALL_FEATURES + BUILDING_FEATURES
    ...
    # --- Save report ---
    report = {
        "experiment": "building_features_re_enablement",
        "features_a": "ALL_FEATURES",
        "features_b": "ALL_FEATURES + BUILDING_FEATURES",
        "n_building_features": len(BUILDING_FEATURES),
    ...
    report_path = report_dir / "experiment_building_features.json"
