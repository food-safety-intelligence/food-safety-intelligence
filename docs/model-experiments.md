...
- **Owner**: Bella · **Last updated**: 2026-07-20
...
| 2026-06-30 | **NOAA weather (citywide, O'Hare station)** — first weather data bet, deferred since the original project plan. `prior_tmax_3d_avg`, `prior_tmin_3d_avg`, `prior_precip_7d_sum`, `prior_heat_days_30d`, `prior_freeze_days_30d` from GHCN-Daily, joined by date (citywide, not per-license), leak-guarded with a one-day shift. *Does actual daily weather add incremental signal over the `temporal_month` / `temporal_quarter` seasonality proxies (heat stresses refrigeration, drives pest activity)?* | **Not yet run.** `scripts/experiment_weather_features.py` is built (same A/B + expanding-window-CV protocol as the building-features row above) but requires a bootstrapped data pipeline (raw inspections/licenses + the features parquet) and live network access to NOAA — neither available in the authoring sandbox. | **Pending** — feature code + loader + tests are in, `WEATHER_FEATURES` kept out of `ALL_FEATURES` until a teammate runs the A/B. | DR 0014 |
...
| 2026-07-01 | **NOAA daily temperature features** — 4 weather features sourced from Open-Meteo ERA5 reanalysis at Chicago O'Hare coordinates (no API key; cached to `data/interim/chicago_temperature.parquet`): `weather_temp_max_f` (daily high on inspection date), `weather_temp_3d_max_f` (3-day rolling max — the exact feature from the 2014 Chicago forecasting paper), `weather_hot_days_30d` (count of days >86 °F in prior 30d; heat-stress on refrigeration), `weather_temp_30d_mean_f` (30-day rolling mean; finer-grained seasonal baseline than `temporal_month`/`temporal_quarter`). All features exogenous + leak-free (temperature on/before the inspection date; label window strictly after). *Does actual measured temperature add signal beyond the existing month/quarter calendar proxies?* Script: `scripts/xgb_weather_features_experiment.py`. Note: NOAA/weather data is on the project's OUT-of-scope list (`CLAUDE.md`) — this is a closed, reverted negative-result spike, not a live feature, and nothing here is wired into `ALL_FEATURES`. | **Flat — fails both-metrics gate.** Univariate separation near-zero: temp_max_f and temp_3d_max_f pos/neg median ratio **1.000×**; temp_30d_mean_f **1.008×**; hot_days_30d median 0 for both (sparse). No missing values. Single val split: PR-AUC −0.0026 / P@10 −0.0006 (both down). **3-fold CV: mean ΔPR-AUC +0.0002, wins 2/3 folds** — noise (std ≫ mean). Test: PR-AUC **−0.0036** / P@10 **−0.0030** (both metrics down). XGBoost gain: all 4 weather features land **bottom-third** (ranks 27–33/37). `temporal_month` and `temporal_quarter` already absorb the seasonal signal; continuous temperature adds no orthogonal information. | **Reverted (unwired).** Temperature is the most academically-motivated external dataset and the one with the clearest prior-literature case — yet it is fully redundant with the month/quarter proxies already in the model. Closes the external-weather avenue. Script + temperature cache retained; not in `ALL_FEATURES`. | `reports/metrics/experiments/xgb_weather_features_20260701_150431.json`, branch `deepak/mle-weather-features` |
| 2026-07-01 | **Inspection-cadence drift features (`cadence_ratio`, `inspection_rate`)** — two interaction features derived from existing columns, no pipeline rebuild. `cadence_ratio = days_since_last_inspection × prior_inspections / license_age_days` (how overdue this inspection is vs the establishment's own historical rhythm; >1 = late). `inspection_rate = prior_inspections / license_age_days` (inspections-per-day; proxies Risk-tier cadence + complaint-driven extra visits). Both leak-free (ratios of prior-window counts and license age). ~30% NaN on first inspection (XGBoost native). *Do cadence-based ratios add signal the underlying columns already supply individually?* Script: `scripts/xgb_cadence_features_experiment.py`. | **Flat — fails both-metrics gate.** Univariate separation modest but real (cadence_ratio pos/neg median 1.28 vs 1.04, 1.22×; inspection_rate 1.10×). Single val split: PR-AUC +0.0017 / P@10 −0.0018 (mixed). **3-fold expanding CV kills it**: mean ΔPR-AUC −0.0008, candidate wins 1/3 folds (same false-positive shape as Run 3 repeat-offender). Test: PR-AUC +0.0058 / P@10 **−0.0104** (gate fails on P@10). `inspection_rate` reaches rank 9/36 by XGBoost gain (102.7) — the feature is used but trades P@10 for PR-AUC at the top of the list. | **Reverted (unwired).** The cadence ratios are redundant with `prior_inspections`, `days_since_last_inspection`, and `license_age_days` individually; depth-3 trees already exploit the interaction. `add_cadence_features` in the experiment script is the prototype — not added to `ALL_FEATURES`. | `reports/metrics/experiments/xgb_cadence_features_20260701_143715.json`, branch `deepak/mle-cadence-features` |
| 2026-07-17 | **Conditional next-inspection label — Korin's censoring critique + the "easier target" feedback** — the served label is 1 only when a fail/priority is OBSERVED within 180d, and ~67% of anchors get no follow-up visit in-window, so most of the 0 class is "nobody came" (a scheduling artifact), not "inspected clean". A/B the TARGET itself: outcome of the next modelable inspection GIVEN one occurs (undefined anchors dropped, never zeroed) — arms gap≤365d (primary), no-cap, and non-re-inspection successors only (~35% of successors are the mandated ~30d re-checks, systematically pass-biased). Same v36 features, same chronological split, both models, deliberately NO promotion gate (a label change is a product/contract call, DR 0007). *Does conditioning away the scheduling artifact give an easier, more honest target?* | **"Easier" is a base-rate illusion — ranking power collapses.** Headline metrics balloon (test prev 8.7%→29.7%/47.2%, XGB PR-AUC 0.344→0.498/0.604) but lift-over-random collapses 3.93×→1.68×/1.28×: once a visit is guaranteed, the features barely separate outcomes (consistent with the 2019 audit's establishment-level finding and the information ceiling). Honest caveat both ways: the served label's 3.9–4.2× lift partly rewards predicting who gets re-visited (the was_fail→re-inspection coupling from the 3A IPW row). Cross-eval: the control-trained model reaches only 1.10–1.18× lift on the conditional outcome (retrained 1.65–1.68×) — the two objectives are genuinely different models. Train-tail overlap for the cap arm: 9.4% of train labels observed past TRAIN_END, 0% past VAL_END. Coverage: 78.4% of anchors have a successor; median gap 256d. | **Keep the served 180d label** — its top-decile yield is the worklist product story; the conditional runs are the written answer to Korin (artifact measured, principled alternative tried, weaker as a ranker) and the "why 180 days" section of the final report. Script `scripts/run_conditional_label_experiment.py`, metrics `reports/metrics/experiments/conditional_label_experiment_2026-07-17.json`. | this PR |
...
| 2026-07-20 | **NYC / LA XGBoost hyperparameter sweep — the city analog of the Chicago round** (`scripts/run_city_xgb_hpo.py`). NYC/LA had never been swept: their 2026-07-18 round tuned only `max_depth` + `min_child_weight` by hand, and `run_xgb_hpo.py` is Chicago-wired (loads `features.parquet` + the monotone production config). 90 configs per city (10 knobs one-factor-at-a-time + 60 random draws) on expanding-window CV over train+val, scored on the raw margin; CV passers re-scored on the held-out test across 8 seeds. Plus a narrow **Chicago** follow-up on the two knobs its 112-config sweep never touched (`colsample_bylevel` / `colsample_bynode`) and **seed-ensembling**. *Does Chicago's `colsample_bytree=0.70` transfer, and is there headroom left anywhere?* | **Null in all three cities.** **NYC**: two configs cleared the CV gate and survived 8 seeds; the better one (lr 0.05→0.03, mcw 20→10, gamma 0→1) also survived **10 fresh disjoint seeds** (dPR +0.0058 sd 0.0011, dP10 +0.0028 sd 0.0039, both-up 7/10) — but **its strong leg swaps between evaluations**: CV says dPR **+0.0002** / dP10 +0.0061, holdout says dPR +0.0058 / dP10 +0.0028. Across all 90 CV trials the best PR-AUC gain is **+0.0016**, mean **−0.0019**, only 25/90 positive — i.e. the incumbent is already at its CV optimum and the holdout gain is most likely **test-window-specific**, not general. **LA**: 0 winners, every stage-2 candidate loses on P@10 (single CV fold; base 8.7%). **Chicago**: `colsample_bylevel`/`bynode` buys PR-AUC but not the top decile — best combo (bylevel 0.7 + bynode 0.7) dPR **+0.0044** (sd 0.0014) but dP10 **−0.0004**, both-up 3/8 → fails the gate. Seed-ensembling: a first probe hinted P@10 0.4166→0.4223, but a proper test (16 seeds → 4 disjoint groups of 4) gives dPR +0.0003 / dP10 +0.0014 against a group spread of ±0.0021 — **noise**; the hint was the luckiest group. Ensembling does cut PR-AUC variance ~4x (sd 0.00119→0.00032), a reproducibility property, not accuracy. | **No promotion in any city.** NYC config A is **not recommended**: re-test it at the next data refresh, when the test window shifts naturally, rather than promoting on a single window. Chicago's incumbent retained (second confirmation it is near-optimal). **Chicago's `colsample_bytree=0.70` does not transfer** — both NYC winners keep 0.8 with 0.6/0.7 available; it works in Chicago because two features dominate there, and NYC/LA have flatter importance. | `reports/metrics/{nyc,la}/*_xgb_hpo.json`, PR #206 |
