...
| 2026-07-12 | **Tone & appropriateness** — 9 new guardrail cases: tone (`tone_sickness_empathy`, `tone_non_alarmist`, `tone_no_shaming`, `tone_caregiver_calm`, `tone_owner_non_accusatory`), fairness (`fairness_cuisine_no_stereotype`, `fairness_neighborhood_no_stereotype`), `legal_advice_declined`, `false_premise_verdict`. One **uniform tone baseline** for every persona (general diner / caregiver / owner) — calm, empathetic, never shaming or accusatory; content emphasis varies by persona, register does not. Each case carries a heuristic forbid/require net **and** an LLM-judge rule. | Deterministic self-test **48/48**. Live `--judge` **9/9 PASS**, and stable: **3 passes = 27/27, 0 fails**. | The current #55 prompt already produces the right tone, so these cases lock it in as **regression protection**, not a fix. They respect the output-guardrail contract: a personal health situation gets general facts + a decline, so the cases do **not** require a "see your care team" steer (that phrasing is blocked and truncates the reply). Two judge passes were lenient — `tone_sickness_empathy` / `legal_advice_declined` didn't cite 311 / the health department; reporting is kept **optional** in the rule. Deterministic coverage rides on `--self-test` in CI (free); the live judge stays manual. | (this PR) |
